...
void binaryJSON(char *json, char *tag, char *end){
	sprintf(json, "{ \"solicitante\" : \"Sensor\" , \"tag\" : \"%s\" , \"tempo\" : \"%s\" }", tag , end);
	printf(json);
}
...
void conectarSever(int* sockfd){
   ...
   *sockfd = socket(AF_INET, SOCK_STREAM, 0);
    ...
    servaddr.sin_addr.s_addr = inet_addr("172.16.103.220");
...
if (TMR_ERROR_TAG_ID_BUFFER_FULL == ret)
  ...
  checkerr(rp, ret, 1, "fetching tag");
...
#else
    {
      ...
      end += snprintf(end, timeEnd-end, ".%03d", micros/1000);
      end += strftime(end, timeEnd-end, "%z", localtime(&seconds));
    ...
    char json[100];
    binaryJSON(json, epcStr, timeStr);
    func(sockfd,json);
