...
public class ControladorCorrida {
     ...
     *
     * @param jogadores ArrayList com os jogadores que irão participar da
     * corrida
     ...
     * @throws TagInvalidaException caso a tag não foi cadastrada no sistema ou
     * na partida atual
     * @throws execoes.VoltaInvalidaException caso a volta não foi instanciada
     * ainda
        ...
        if (rodando && !corridaConcluida) {
            ...
            if (jogador != null) {
                ...
                if (validarVolta(jogador, voltaComputada)) {
                    ...
                    System.out.println("ID Jogador: "+jogador.getID() + "Volta: "+jogador.getVolta() + "Carro: " + jogador.getCarro().getCor());
                } else {
                    throw new VoltaInvalidaException("O sensor coletou uma volta inválida e não foi computada no sistema");
                }

            } else {
                throw new TagInvalidaException("A tag não está cadastrada no sistema ou na corrida atual");
            }

        } else {
            throw new CorridaNaoIniciadaException("A corrida já foi finalizada ou não foi inicializada");
        }
    }

    /**
     * Para a volta ser válida o sensor deve coletar a mesma tag no mínimo 6
     * segundos após a ultima passagem do mesmo jogador
     *
     * @param joador jogador pertencene a tag que foi coletada
     * @param voltaComputada o tempo em que a tag foi coletada a partir do
     * cronometro interno
    ...
    /**
     * Verifica se a tag coletada é a volta mais rápida do jogador, e muda se
     * for verdadeira ela também implemeta a primeira volta do jogador è setado
     * a variação da volta como volta mais rápida
     *
    ...
    /**
     * Veriica se o jogador que acabou de passar fez uma volta mais rápida que a
     * registrada
     *
     * @param jogadorAtual jogador coletado pela tag
     */
    private void verificaMenorVoltaDaCorrida(Jogador jogadorAtual) {
       if(jogadorDavoltaMaisRapida != null){
    ...
    /**
     * O método de ordenação primeiramente remove o jogador da lista e jogadores
     * do jogo, e depois o realoca para a posição logo após o ultimo jogador que
     * passou na mesma volta que ele acabou de dar. Sendo assim, caso ninguém
     * tenha passado naquela volta ainda e o jogador atual foi o pioneiro, ele
     * sera realocado para o primeiro lugar do array. Ele também chama o método
     * para averiguar se a corrida já terminou
     *
    ...
    /**
     * Checa se o jogador ficou em ultima posição e se completou a quantidade de
     * voltas précadastradas
     *
     * @param jogador jogador coletado
